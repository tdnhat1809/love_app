...
try {
  ...
  window.updatePartner=function(d){if(!d||!d.lat||!d.lng)return;var p=[d.lat,d.lng];var spd=d.speed||0;var bat=d.battery!=null?d.battery:-1;var net=d.networkType||'';var pn=d.placeName||'';var st=pn||'\u1ede y\u00ean';if(!pn){st=spd>80?'\u0110ang l\u00e1i nhanh':spd>40?'Di chuy\u1ec3n':spd>5?'\u0110i b\u1ed9':'\u1ede y\u00ean'}var bs=bat>=0?'\ud83d\udd0b '+bat+'%':'';var h='<div class="avatar-bubble"><div class="avatar-circle">';if(d.avatarUrl)h+='<img src="'+d.avatarUrl+'">';else h+=(d.emoji||'\ud83d\udc67');h+='<span class="heart-badge">\ud83d\udc95</span></div><div class="info-card"><div class="info-name">'+(d.name||'Ng\u01b0\u1eddi y\u00eau')+'</div><div class="info-status">'+st+'</div><div class="info-meta">';if(bs)h+='<span>'+bs+'</span>';h+='</div></div></div>';var ic=L.divIcon({className:'',html:h,iconSize:[90,90],iconAnchor:[45,45]});if(window.partnerMarker){window.partnerMarker.setLatLng(p);window.partnerMarker.setIcon(ic)}else{window.partnerMarker=L.marker(p,{icon:ic}).addTo(map)}map.setView(p,map.getZoom(),{animate:true})};
  window.updateMe=function(d){if(!d||!d.lat||!d.lng)return;var p=[d.lat,d.lng];var h='<div class="avatar-bubble"><div class="avatar-circle me">';if(d.avatarUrl)h+='<img src="'+d.avatarUrl+'">';else h+=(d.emoji||'\ud83d\udc66');h+='</div><div class="info-card"><div class="info-name">T\u00f4i</div></div></div>';var ic=L.divIcon({className:'',html:h,iconSize:[70,70],iconAnchor:[35,35]});if(window.myMarker){window.myMarker.setLatLng(p);window.myMarker.setIcon(ic)}else{window.myMarker=L.marker(p,{icon:ic}).addTo(map)}};
